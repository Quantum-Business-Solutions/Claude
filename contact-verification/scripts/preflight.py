...
REQUIRED={
 ...
 'writeverdicts.py':[('sys.exit(2)',             'fail-fast on non-2xx'),
                     ...
                     ('def identity_doubt(',    'slug-vs-name check on every verdict'),
                     ('no_destination',         'a `no` must record where they went or why not'),
                     ('identity_ok',            'identity doubt BINDS the verdict, not just the title')],
 'movepipe.py':     [('associations read failed','associations guard'),
                     ...
                     ('def dest_status(',        'destination customer/deal/meeting check'),
                     ('previous employer == destination',
                                                 'refuses to record the destination as the employer left'),
                     ('propertiesWithHistory=associatedcompanyid',
                                                 'previous employer recovered after the ejection workflow detaches it')],
 'twolists.py':     [('ai__reassociated_on',     'Moved-Companies filters the date')],
 'recoverdest.py':  [('def negated(',           'a negation is not a destination'),
                     ('contains a negation',    'nor is a capture containing one')],
