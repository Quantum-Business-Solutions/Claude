...
| ID | Where it lives |
|---|---|
| QBS Company Record ID | QBS portal 20682069 (search by company name) |
| Client HubSpot Portal ID | client's portal |
| Portal access | Client Command stored credential (preferred) or `CLIENT_HUBSPOT_TOKEN` env var — see SKILL.md doctrine #5 |
...
expected client — wrong portal = stop. If scopes are missing, list which
ticket types can't be verified and flag those `Needs Review` rather than
skipping silently. Scopes needed for full verification:
`crm.objects.{companies,contacts,deals}.read`,
...
Evidence bar: the three-check rule in `references/evidence-standards.md`.

Outcomes: `✅ DONE` (close candidate w/ evidence) · `🟢 DONE-ON-CALL` (Phase
3.5) · `🟡 PARTIAL` (artifact exists, gaps remain) · `🔴 OPEN` · `⛔ BLOCKED`
(external dependency — record the blocker, don't verify further).

## Phase 3.5 — Same-call completion hunt

For every 🔴 OPEN ticket: find the meeting nearest its `createdate` (±14
days) via `list_meetings`, search the transcript for completion phrases, and
cross-check the portal for a matching artifact inside the canonical window
(meeting start → meeting end + 24h). All three signals + QBS attribution →
...
- If the SOW has a Scope Burn Notice threshold (Platinum: 75%), check whether
  ...
  would push past it, flag BEFORE execution (doctrine; also in mode-close).

## Phase 6 — Produce the report

Structure: `references/output_template.md`. Also write the four flag
properties on every evaluated ticket so the review can happen in HubSpot as
well as in the report. Then stop — closes go through
`references/mode-close.md` after the user approves.
