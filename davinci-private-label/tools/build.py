...
def inner_html(h):
    ...
    body = h.strip()
    # A leading rule is decoration above the section, not the section's wrapper.
    # `wrap_style` already steps over one to find the wrapper and read its
    # padding; this did not, so on `book-consultation` the module was given
    # V1's own padded wrapper *and* the padding read off it, and the block sat
    # 26px lower than V1 draws it. The rule itself is copy and is kept.
    lead = ''
    m = re.match(r'\s*(?:<(?:hr|br)\b[^>]*>\s*)+', body)
    if m:
        lead, body = m.group(0).strip(), body[m.end():]
    for _ in range(2):
        m = re.match(r'\s*<div\b([^>]*)>\s*', body)
        if not m: break
        # A wrapper that declares only the section's alignment used to be kept,
        # because stripping it sent centred copy left. The one caller reads that
        # alignment into the module's own `align` field, so keeping the wrapper
        # no longer buys anything and costs a measure: V1's `max-width: 900`
        # sat inside it, the module applied `max_width: 900` again outside it,
        # and its 20px of padding then ran the copy at 860 -- forty pixels
        # narrower than V1 on every section written this way.
        # A wrapper that lays its children out -- grid or flex -- is a shape the
        # module cannot reproduce and has to be carried through as it stands.
        if re.search(r'display:\s*(?:grid|flex)', m.group(1)): break
        rest = body[m.end():]
        j = rest.rstrip().rfind('</div>')
        if j < 0: break
        body = rest[:j]
    return (lead + body).strip() if lead else body.strip()
...
def keep_type(attrs, paint=None):
    ...
    for d in re.split(r';', m.group(1) if m else ''):
        ...
        if k in TYPE_DECLS and ':' in d:
            out.append(d.strip())
    # A declaration V1 leaves out is not "whatever the module does": an
    # undeclared line-height on a <p> is Main.css's 27px, and on anything else
    # it is `normal`. Saying so explicitly is the only way the module's own 1.4
    # or 1.65 stops winning. `paint` is the tag V1 used.
    if paint and not any(d.startswith('line-height') for d in out):
        out.append('line-height: %s' % ('27px' if paint == 'p' else 'normal'))
    return '; '.join(out)
...
def button_pad(h, dflt=""):
    """The padding V1 writes on the section's own call-to-action anchor."""
    m = re.search(r'<a[^>]*\sstyle="([^"]*)"', h)
    if not m: return dflt
    p = re.search(r'padding:\s*([^;"]+)', m.group(1))
    return p.group(1).strip() if p else dflt


def lh_of(attrs, dflt=None, paint='p'):
    """The line-height a reader actually sees on a V1 element.

    A declaration on the element wins. Where V1 declares none, a <p> takes
    Main.css's `p{line-height:27px}` and everything else takes `normal` --
    which is why the modules' own 1.1, 1.25, 1.35 and 1.4 made kickers, stat
    labels, FAQ questions and three section headings a few pixels shorter or
    taller than V1 and walked everything below them up or down the page.
    `dflt` is returned only when there is no element to read at all."""
    if not attrs: return dflt
    m = re.search(r'line-height:\s*([0-9.]+(?:px|em|rem)?)', attrs)
    return m.group(1) if m else ('27px' if paint == 'p' else 'normal')


def heading_lh(h, dflt=None):
    """The leading on the section's own heading, as V1 paints it."""
    m = headline_el(h)
    return lh_of(m.group(2), dflt, 'h2') if m else dflt


def card_title_lh(h, dflt=None):
    """The leading on a card's <h3>, as V1 paints it."""
    m = re.search(r'<h3(?=[\s>])([^>]*)>', h)
    return lh_of(m.group(1), dflt, 'h3') if m else dflt
...
def cell_density(h):
    """`compact` where V1 draws its tighter product card, `standard` otherwise.

    The card module has two card sizes and the reader never chose between them,
    so every grid came out at the standard one: 34x30 of padding, a 56px icon
    tile, a 19px title, 15px body and a 13px eyebrow. V1's product-list card is
    the compact one, to the pixel -- 28x26, a 48px tile, 18/14/11 -- and the
    module's compact class already carries all five numbers. The padding is what
    identifies it: the individual sizes are separate fields on the standard card
    but are hard-coded on the compact one, so this has to be read from the box,
    not from the type."""
    cells = grid_cells(h)
    m = re.match(r'\s*<\w+[^>]*\sstyle="([^"]*)"', cells[0]) if cells else None
    pm = re.search(r'padding:\s*(\d+)px\s+(\d+)px', m.group(1)) if m else None
    return "compact" if pm and (int(pm.group(1)), int(pm.group(2))) == (28, 26) else "standard"


def hold_links(html):
    """Keep an inline link at the weight V1 draws it.

    Every module bolds the links inside its rich text -- `.pl-rt__inner a`,
    `.pl-cg__content a`, `.pl-cta__content a` and five more all declare
    `font-weight: 700`. V1's inline links declare a colour and an underline and
    nothing else, so they inherit their paragraph's 400 and read as ordinary
    body copy. None of the modules has a field for it and the rule carries no
    !important, so restating V1's own weight on the element is what holds it.
    A link that already declares a weight -- every button V1 draws does -- is
    left exactly as it is."""
    def one(m):
        tag, attrs = m.group(0), m.group(1)
        if re.search(r'font-weight\s*:', attrs, re.I):
            return tag
        s = re.search(r'\sstyle="([^"]*)"', attrs)
        if s:
            body = s.group(1).rstrip().rstrip(';')
            return tag.replace(s.group(0), ' style="%s; font-weight: 400"' % body)
        return '<a style="font-weight: 400"' + attrs + '>'
    return re.sub(r'<a\b([^>]*)>', one, html)
...
def section(kind, h, w):
    ...
    if kind == 'hero':
        ...
        p = {**base(scr_col), "module_id": MOD['hero'], "align": align_of(h, "center"),
             ...
             # V1's own button box. The module hard-coded the common
             # 16px 38px, and the one band V1 draws at 14px 32px came out
             # twelve pixels wider and five taller than the page shows.
             "button_padding": button_pad(h)}
        if bgimg:
    ...
    if kind == 'richtext':
        ...
        htype = keep_type(hm.group(2), 'h2') if hm else ''
        # the heading sits in the rich-text flow, where the module adds no gap of
        # its own, so V1's own margin can come across with it -- 26px against the
        # module's 24
        hmar = re.search(r'margin:\s*([^;"]+)', hm.group(2)) if hm else None
        hstyle = '; '.join(x for x in (htype, '' if 'font-size' in htype
                                       else f'font-size: {hpx}px',
                                       f'margin: {hmar.group(1).strip()}' if hmar else '') if x)
    ...
    if kind == 'sectionheader':
        return {**base("#FFFFFF"), "module_id": MOD['sh'], "align": align_of(h),
                "headline_size": headline_px(h, 34), "headline_lh": heading_lh(h, "1.25"),
                "eyebrow": eyebrow(h),
    ...
    if kind == 'cardgrid':
        ...
        p = {**base("#FFFFFF"), "module_id": MOD['cg'], "max_width": bound,
             ...
             # V1's card heading declares no leading, so it sits on a normal
             # line box; the module's 1.3 made every card three pixels taller
             "title_lh": card_title_lh(h, "1.3"),
    ...
    if kind == 'statband':
        ...
        for cell in cells:
            ...
            if len(ps) > 2:
                # carried with its own type. The module paints the description
                # 14px in rgba(255,255,255,.8) on a dark band and has no field
                # for either; V1's third line is 13px in solid white with the
                # fade applied as opacity on the element. The field is rich text
                # rendered as authored, so V1's own declarations are what hold.
                st["description"] = ''.join(
                    ('<p style="%s">%s</p>' % (keep_type(a, 'p'), i.strip()))
                    if keep_type(a, 'p') else '<p>%s</p>' % i.strip()
                    for a, i in ps[2:])
        ...
        return {**base("#012638"), "module_id": STAT,
                ...
                "headline_size": headline_px(h, 32), "headline_lh": heading_lh(h, "1.25"),
                "section_subhead": subhead_of(h), "subhead_size": sub_px(h),
                "subhead_width": subhead_maxw(h, 0),
                "subhead_color": {"color": subhead_colour(h), "opacity": 100},
    ...
    if kind == 'form':
        ...
        return {**base("#f7f7f6"), "module_id": FORM, "layout": layout,
                ...
                "headline_lh": lh_of(hm.group(2) if hm else '', "1.25", 'h2'),
                # V1's form band is a single 720px column inside a 24px
                # gutter; the module capped its copy at 850 and its form at
                # 600 and took the gutter from every other section's 20px
                "max_width": maxw(h, 0),
                "gutter": px_of(s, r'padding:\s*\d+px\s+(\d+)px', 20),
...
def _hold_links_deep(o):
    """Apply `hold_links` to every piece of markup a module is handed.

    Rich text reaches the modules through a dozen different fields -- `content`,
    `section_subhead`, `subhead`, a card's `content`, a stat's `description` --
    and doing this at each of them is how one gets missed. The link fields are
    dicts of href and target, never markup, so they are not touched."""
    if isinstance(o, dict):
        return {k: (v if k in ('link', 'url', 'image', 'form_field') else _hold_links_deep(v))
                for k, v in o.items()}
    if isinstance(o, list):
        return [_hold_links_deep(v) for v in o]
    if isinstance(o, str) and '<a ' in o:
        return hold_links(o)
    return o
...
def build(page):
    ...
    for w in ws:
        ...
        if k == 'skip':
            continue
        out.append((k, _hold_links_deep(section(k, h, w))))
    return out


def v1_source(v1id):
    """V1 as the browser renders it -- the draft buffer, not the published object.

    Nothing on this site is published, so `/site-pages/{id}` returns a stale
    object left over from the page's first save while `?hs_preview=` -- which is
    what the reference capture renders and therefore what the rebuild is judged
    against -- serves the draft. On four pages the two disagree: `privacy` still
    reads `[BRAND_TBD]` in the published object where the draft says `Prexera`,
    and `get-started` has no headHtml there at all while its draft carries the
    2.6kB of CSS that sizes and centres its form. Building from the published
    object rebuilt copy the page does not show and dropped that stylesheet.
    Falls back to the published object for a page that has no draft buffer."""
    try:
        d = req(f"{API}/cms/v3/pages/site-pages/{v1id}/draft")
    except RuntimeError:
        return req(f"{API}/cms/v3/pages/site-pages/{v1id}")
    if (d.get('widgetContainers') or {}).get('main_content', {}).get('widgets'):
        return d
    return req(f"{API}/cms/v3/pages/site-pages/{v1id}")
...
def main():
    ...
    # The draft buffer and the published object do not always agree on the slug
    # -- `ads-contract-mfg` is `en/pl-demo-ads-contract-mfg` in its draft and
    # `pl-demo-ads-contract-mfg` published -- and the rebuilt page was named
    # from whichever one the reader happened to hold. Searching for both is what
    # finds the existing draft instead of quietly minting a second one beside it.
    live_slug = req(f"{API}/cms/v3/pages/site-pages/{v1id}").get('slug')
    cands, seen = [], set()
    for sl in (v1['slug'], live_slug):
        if sl and sl not in seen:
            seen.add(sl); cands.append(sl + '-v3')
    slug = cands[0]
    # v3_ids.json is the record of which draft belongs to which V1 page, and it
    # is the only thing that survives a slug that has moved. A slug search alone
    # answered with whichever page happened to match first, which on the second
    # spelling of the slug is a page nobody is measuring.
    known = json.load(open(S + 'v3_ids.json')) if os.path.exists(S + 'v3_ids.json') else {}
    v3id = known.get(str(v1id))
    if v3id:
        try:
            req(f"{API}/cms/v3/pages/site-pages/{v3id}")
        except RuntimeError:
            v3id = None
    for c in cands:
        if v3id: break
        found = req(f"{API}/cms/v3/pages/site-pages?slug={urllib.parse.quote(c)}")
        if found.get('total'):
            v3id = sorted(r['id'] for r in found['results'])[0]
    if v3id:
        pass
    else:
