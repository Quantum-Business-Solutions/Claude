...
# Public DaVinci URLs that will 301 to each Praxera page at cutover. The v1 "pl-demo" pages
# are the SAME HubSpot records as today's Praxera pages (re-slugged, moved domain) and the -v3
# pages were QBS working copies (now archived) — neither was ever public, so neither is an
# "original". compare_pairs.json holds the true public sources; map them to current slugs.
import glob as _g
_v1={json.load(open(f))["slug"]:str(json.load(open(f))["id"]) for f in _g.glob("snapshots/v1-pre-promotion/*.json")}
_byid={str(x["id"]):x["slug"] for x in ids["pages"]}
ORIG={}
for _d in R("compare_pairs"):
    if _d["old"].get("state")=="GONE": continue
    _cur=_byid.get(_v1.get(_d["new_slug"],""))
    if _cur is None: continue
    ORIG.setdefault(_cur,[]).append(_d["old_url"])
_rank=lambda u:(0 if "//www." in u else 1 if "//info." in u else 2, u)
for _k in ORIG: ORIG[_k]=sorted(dict.fromkeys(ORIG[_k]),key=_rank)
...
def seeds_for(slug):
    ...
    for n,c in enumerate(x for x in hcom["comments"] if x["slug"]==slug):
        o={"id":f"hs:{slug}:{n}","n":c["n"],"at":c["at"],"t":c["t"],"src":c.get("src","HubSpot page comment")}
...
for p in sorted(pairs["pages"],key=lambda x:x["slug"] or ""):
    ...
    olds=ORIG.get(slug,[])
    if len(olds)>1: i.append(["mute",f"{len(olds)} DaVinci URLs redirect here","At cutover all of these 301 to this page: "+" · ".join(host(u) for u in olds)])
    if not olds: i.append(["mute","nothing redirects here","No public DaVinci page maps to this one — it was built new for Praxera and only gets traffic from new links."])
    add("p",slug or "home",slug or "(home)",r=(host(olds[0])+(f" (+{len(olds)-1} more)" if len(olds)>1 else "")) if olds else "",
        t=host(olds[0]) if olds else "",i=i,new=not olds,
...
for slug,hp in hsp.items():
    ...
    add("p",slug,slug,r="",i=health_chips(pageh.get(slug),"page")+[["mute","added since 31 Aug","Not in the original migration pair list."],["mute","nothing redirects here","No public DaVinci page maps to this one — it was built new for Praxera and only gets traffic from new links."]],
...
# ---- chunks -----------------------------------------------------------------
# Seed the interned tables from the last build so indices in unchanged data sections stay valid
# (a data section is only re-uploaded when its own bytes change).
try:
    _prev=[x for x in json.load(open("deliverables/signoff_chunks.json")) if x["block_key"]=="metadata"][0]["body"]
    _prev=json.loads(re.search(r">(.*)</script>",_prev,re.S).group(1))
    labels=[list(x) for x in _prev.get("labels",[])]; types=list(_prev.get("types",[]))
except Exception: labels=[]; types=[]
idx={tuple(x):n for n,x in enumerate(labels)}
for r in rows:
    out=[]
    for x in r["i"]:
        k=tuple(x)
