...
window.addEventListener("message",function(e){
  ...
  if(d.type==="state")onRemote(d.key,d.value);
    ...
    if(user.name)who=user.name; if(user.kind==="team")side="qbs"; if(user.kind==="client"&&!side)side="client";
    render();}
});
function requestState(k){post({type:"ready",key:k});}
var wq=[],wbusy=false,wt=null;
function writeState(k,v,cb){
  var s=JSON.stringify(v);
  if(s.length>CAP){flag("too much saved under "+k+" — tell QBS","err");return;}
  wq=wq.filter(function(x){return x.k!==k;});wq.push({k:k,v:v,cb:cb});pump();
}
/* One save in flight at a time. The host coalesces saves for 600ms; sending two
   keys inside that window used to keep only the last one, so history went missing. */
function pump(){if(wbusy||!wq.length)return;var x=wq.shift();wbusy=true;
  saveCb[x.k]=function(ok,err){wbusy=false;clearTimeout(wt);if(x.cb)x.cb(ok,err);pump();};
  post({type:"state:set",key:x.k,value:x.v});
  wt=setTimeout(function(){wbusy=false;pump();},2500);}
...
function mergeItem(l,r){if(!l)return r;if(!r)return l;var o={};
  o.q=mergeStamp(l.q,r.q);o.c=mergeStamp(l.c,r.c);o.g=mergeStamp(l.g,r.g);o.f=mergeStamp(l.f,r.f);
...
function statusOf(g,id){var it=state[g][id]||{};return {q:stat(it,"q"),c:stat(it,"c"),g:stat(it,"g"),f:stat(it,"f")};}
var REVS=["r1","r2","r3"];
...
function comments(r){var it=state[r.k][r.id]||{},sr=it.sr||{};
  var seeds=(r.sc||[]).map(function(s){var o={id:s.id,n:s.n,at:s.at,t:s.t,src:s.src,seed:1};
    var x=sr[s.id]||{};o.st=x.st||"open";o.rs=x.rs;o.gl=x.gl;o.re=(s.re||[]).map(function(r,i){return {id:s.id+":r"+i,n:r.n,t:r.t,at:r.at,seed:1};}).concat(x.re||[]);return o;});
...
function makeGlobal(g,id,cid){var r=findRow(g,id);if(!r)return;
  ...
  gi.cm.push({id:uid(),n:me(),sd:side,t:"[From "+r.n+" — "+c.n+"] "+c.t,at:now(),st:"open",re:[],from:{g:g,i:id,n:r.n}});
  logIt("make-global",g,id,r.n,c.t.slice(0,140));touch(gkey(g));touch(KEY.gen);}
...
function csvText(){var q=function(v){v=String(v==null?"":v);return /[",\n]/.test(v)?'"'+v.replace(/"/g,'""')+'"':v;};
  var out=[["Group","Asset","Type","Live link","HubSpot link","Detail","Findings","QBS approved by","QBS approved at","Client approved by","Client approved at","Regulatory approved by","Regulatory approved at","Needs work by","Needs work at","Rev 1","Rev 2","Rev 3","Open comments","Comments","Note","Removed"].join(",")];
  GROUPS.forEach(function(g){rowsOf(g[0]).forEach(function(r){var s=statusOf(r.k,r.id);
    ...
    out.push([g[1],r.n,r.y||"",r.u||"",r.h||"",r.r||"",r.i.map(function(x){return x[1]+(x[2]?" — "+x[2]:"");}).join("; "),
      s.q?s.q.by:"",s.q?s.q.at:"",s.c?s.c.by:"",s.c?s.c.at:"",s.g?s.g.by:"",s.g?s.g.at:"",s.f?s.f.by:"",s.f?s.f.at:""].concat(REVS.map(function(k){var x=revOf(r,k);return x?"✓ "+x.by:"";})).concat([openCount(r),cs,r.note||"",r.deleted?"yes":""]).map(q).join(","));});});
  return out.join("\n");}
...
function xlsxBook(X){
  ...
  GROUPS.forEach(function(g){
    ...
    var aoa=[[cell(META.title.toUpperCase()+" — "+g[1].toUpperCase(),T)],[cell((META.brandline||"")+"  |  exported "+new Date().toLocaleString()+"  |  Rev ticks and approvals as recorded in ClientCommand",S)],[],
      ["Asset","Type","Live","HubSpot editor",g[2]||"Detail","Rev 1","Rev 2","Rev 3","QBS approved","Client approved","Regulatory approved","Needs work","Findings","Open comments","Comments","Note"].map(function(h){return cell(h,H);})];
    var byType={};rows.forEach(function(r){(byType[r.y||"Other"]=byType[r.y||"Other"]||[]).push(r);});
    var types=Object.keys(byType).sort();var merges=[{s:{r:0,c:0},e:{r:0,c:15}},{s:{r:1,c:0},e:{r:1,c:15}}];
    types.forEach(function(ty){if(types.length>1){aoa.push([cell("▸  "+ty.toUpperCase(),G)].concat(Array(15).fill(cell("",G))));merges.push({s:{r:aoa.length-1,c:0},e:{r:aoa.length-1,c:15}});}
      byType[ty].forEach(function(r){var s=statusOf(r.k,r.id);var cs=comments(r);
        ...
        aoa.push([cell(r.n,B),cell(r.y||"",B),link(r.u,r.u?"Open":""),link(r.h,r.h?"Open in editor":""),cell(r.r||"",B)].concat(rv).concat([
          s.q?cell("✓ "+stampTxt(s.q),OK):cell("",NO),s.c?cell("✓ "+stampTxt(s.c),OK):cell("",NO),s.g?cell("✓ "+stampTxt(s.g),OK):cell("",NO),s.f?cell(stampTxt(s.f),BAD):cell("",B),
    ...
    ws["!cols"]=[{wch:38},{wch:14},{wch:9},{wch:16},{wch:34},{wch:7},{wch:7},{wch:7},{wch:26},{wch:26},{wch:26},{wch:22},{wch:40},{wch:9},{wch:70},{wch:30}];
  ...
  gc.forEach(function(c){ga.push([cell(c.n,B),cell(c.sd==="qbs"?"QBS":c.sd==="reg"?"Regulatory":c.seed||c.sd==="client"?"Client":"",B),cell(when(c.at),B),cell(c.src||"",B),cell(c.st==="done"?"Resolved":"Open",c.st==="done"?OK:NO),cell(c.t,B),cell((c.re||[]).map(function(x){return x.n+": "+x.t;}).join("\n"),B)]);});
  ...
  log.e.slice().reverse().forEach(function(e){la.push([cell(when(e.at),B),cell(e.by,B),cell(e.sd==="qbs"?"QBS":e.sd==="client"?"Client":e.sd==="reg"?"Regulatory":"",B),cell(ACT[e.a]||e.a,B),cell(e.n,B),cell(e.x||"",B)]);});
...
var view={},open={},editing={},confirmDel={},sel={},showDel={},showLog=false,addOpen={},replyOpen={},collapsed={log:false};var showAllLog=false;
...
function matches(r,v){var s=statusOf(r.k,r.id);
  ...
  switch(v.f){case "all":return true;case "todo":return !s.q&&!s.c&&!s.f;case "qbs":return !!s.q;case "client":return !!s.c;case "reg":return !!s.g;
  ...
  return true;}
var FILTERS=[["all","All"],["todo","Not reviewed"],["qbs","QBS approved"],["client","Client approved"],["reg","Regulatory approved"],["both","Fully approved"],
  ["fix","Needs work"],["open","Open comments"],["issue","Has a finding"],["changed","Edited here"]];
var ACT={"qbs-approve":"QBS approved","qbs-unapprove":"removed QBS approval","client-approve":"Client approved","reg-approve":"Regulatory approved","reg-unapprove":"removed Regulatory approval",
  ...
  "rev-tick":"ticked a revision on","rev-clear":"cleared a revision on","make-global":"made a note global from"};

/* ---- history: the log plus anything the saved stamps prove happened ---------- */
function histEvents(){var seen={},out=[];
  log.e.forEach(function(e){out.push(e);seen[e.a+"|"+e.g+"|"+e.i+"|"+String(e.at||"").slice(0,16)]=1;});
  function add(a,g,i,n,at,by,sd,x){if(!at)return;var key=a+"|"+g+"|"+i+"|"+String(at).slice(0,16);if(seen[key])return;seen[key]=1;
    out.push({id:"d:"+key,at:at,by:by||"",sd:sd||"",a:a,g:g,i:i,n:n,x:x||"",d:1});}
  GK.concat(["_"]).forEach(function(g){var grp=state[g]||{};for(var id in grp){var it=grp[id],name=g==="_"?"Global notes":rowName(g,id);
    [["q","qbs-approve","qbs-unapprove"],["c","client-approve","client-unapprove"],["g","reg-approve","reg-unapprove"],["f","needs-work","clear-flag"]].forEach(function(k){var st=it[k[0]];if(st&&st.at)add(st.x?k[2]:k[1],g,id,name,st.at,st.by,k[0]==="q"?"qbs":k[0]==="c"?"client":k[0]==="g"?"reg":"");});
    REVS.forEach(function(k){var st=it[k];if(st&&st.at)add(st.x?"rev-clear":"rev-tick",g,id,name,st.at,st.by,"","Rev "+k.slice(1));});
    (it.cm||[]).forEach(function(c){add("comment",g,id,name,c.at,c.n,c.sd,(c.t||"").slice(0,140));(c.re||[]).forEach(function(r){add("reply",g,id,name,r.at,r.n,r.sd,(r.t||"").slice(0,140));});
      if(c.rs&&c.rs.at)add(c.st==="done"?"resolve-comment":"reopen-comment",g,id,name,c.rs.at,c.rs.by,"");});
    var sr=it.sr||{};for(var cid in sr){var x=sr[cid];(x.re||[]).forEach(function(r){add("reply",g,id,name,r.at,r.n,r.sd,(r.t||"").slice(0,140));});
      if(x.rs&&x.rs.at)add(x.st==="done"?"resolve-comment":"reopen-comment",g,id,name,x.rs.at,x.rs.by,"");if(x.gl&&x.gl.at)add("make-global",g,id,name,x.gl.at,x.gl.by,"");}}});
  GK.forEach(function(g){var m=edits.mod[g]||{};for(var id in m){for(var f in m[id]){var st=m[id][f];if(st&&st.at)add("edit",g,id,rowName(g,id),st.at,st.by,"",f+": "+String(st.v||"(blank)").slice(0,60));}}
    (edits.add[g]||[]).forEach(function(o){add("add",g,o.id,o.n||"",o.at,o.by,"");});
    var d=edits.del[g]||{};for(var id2 in d){var st2=d[id2];if(st2&&st2.at)add(st2.x?"restore":"delete",g,id2,rowName(g,id2),st2.at,st2.by,"");}
    var ch=edits.chips[g]||{};for(var id3 in ch){for(var lb in ch[id3]){var st3=ch[id3][lb];if(st3&&st3.at)add("clear-chip",g,id3,rowName(g,id3),st3.at,st3.by,"",lb);}}});
  out.sort(function(a,b){return (a.at||"")<(b.at||"")?1:-1;});return out;}
function commentTotals(){var o=0,d=0;GK.forEach(function(g){rowsOf(g).forEach(function(r){if(r.deleted)return;comments(r).forEach(function(c){if(c.st==="done")d++;else o++;});});});
  comments(GEN).forEach(function(c){if(c.st==="done")d++;else o++;});return {open:o,done:d};}
...
function rowHtml(r){
  ...
  var cls2="r"+(s.q&&s.c&&s.g?" done":"")+(s.f?" flag":"")+(r.deleted?" gone":"")+(sel[r.k][r.id]?" sel":"");
  var nm='<td class="nm stk2" title="Double-click a value to edit it"><span class="ttl" data-f="n">'+E(r.n)+"</span>"+(r.y?'<span class="ty" data-f="y">'+E(r.y)+"</span>":'<span class="ty ghost" data-f="y">+ type</span>')
    ...
    +(s.q||s.c||s.g||s.f?"<div>"+stampHtml("QBS",s.q,"q")+stampHtml("Client",s.c,"c")+stampHtml("Regulatory",s.g,"g")+stampHtml("Needs work",s.f,"f")+"</div>":"")+"</td>";
  var rv=r.deleted?'<td class="act"><button class="gh" data-a="restore">Restore</button></td>'
    :'<td class="act"><span class="rv">'
      ...
      +'<button class="ok rg" data-a="g" aria-pressed="'+(!!s.g)+'"'+(locked("g")?' disabled title="Regulatory (or client) only"':"")+'>Regulatory ✓</button>'
...
function sideCls(c){return c.sd==="qbs"?"qbs":(c.sd==="client"||c.sd==="reg"||c.seed)?"client":"";}
function threadHtml(r){
  ...
  return '<tr class="th" data-g="'+r.k+'" data-i="'+E(r.id)+'"><td colspan="7"><div class="panel">'
    ...
    +"</div></td></tr>";}
function tally(rows){var t={q:0,c:0,g:0,both:0,f:0,todo:0,n:0};rows.forEach(function(r){if(r.deleted)return;t.n++;var s=statusOf(r.k,r.id);
  if(s.q)t.q++;if(s.c)t.c++;if(s.g)t.g++;if(s.q&&s.c&&s.g)t.both++;if(s.f)t.f++;if(!s.q&&!s.c&&!s.g&&!s.f)t.todo++;});return t;}
function barHtml(t){var n=t.n||1;return '<i class="a" style="width:'+(t.both/n*100).toFixed(1)+'%"></i><i class="q" style="width:'+((t.q-t.both)/n*100).toFixed(1)+'%"></i><i class="c" style="width:'+(Math.max(0,t.c-t.both)/n*100).toFixed(1)+'%"></i><i class="g" style="width:'+(Math.max(0,t.g-t.both)/n*100).toFixed(1)+'%"></i><i class="f" style="width:'+(t.f/n*100).toFixed(1)+'%"></i>';}
...
function groupHtml(g){var gk=g[0],rows=rowsOf(gk),v=view[gk];
  ...
  var bulk=ns?'<div class="bulk"><b>'+ns+" selected</b>"
    +'<button data-a="bq"'+(locked("q")?" disabled":"")+'>QBS approve</button><button data-a="bc"'+(locked("c")?" disabled":"")+'>Client approve</button><button data-a="bg"'+(locked("g")?" disabled":"")+'>Regulatory approve</button>'
...
var showAbout=false;
function cmtHtml(r,c){var re=(c.re||[]).map(function(x){return '<div class="re"><div class="who"><span class="nm2">'+E(x.n)+"</span>"+(x.sd?"<span>"+sideName(x.sd)+"</span>":"")+"<span>"+when(x.at)+'</span></div><div class="txt">'+E(x.t)+"</div></div>";}).join("");
  var rk=r.k+":"+r.id+":"+c.id;
  return '<div class="cmt '+sideCls(c)+(c.st==="done"?" done":"")+'" data-c="'+E(c.id)+'"><div class="who"><span class="nm2">'+E(c.n)+"</span>"+(c.sd?"<span>"+sideName(c.sd)+"</span>":"")+"<span>"+when(c.at)+"</span>"
    +(c.src?'<em>'+E(c.src)+"</em>":"")+(c.st==="done"?'<b>resolved by '+E(c.rs&&c.rs.by||"")+" "+when(c.rs&&c.rs.at)+"</b>":"")+"</div>"
    +'<div class="txt">'+E(c.t)+"</div>"+re
    +'<div class="cact">'+(c.st==="done"?'<span class="pill done">✓ Resolved</span><button class="gh mini" data-a="reopen">Reopen</button>':'<span class="pill open">Open</span><button class="mini ok2" data-a="resolve">Resolve ✓</button>')
    +'<button class="gh mini" data-a="reply">Reply</button>'+(r.k!=="_"?(c.gl?'<span class="pill gl" title="Copied to Global notes by '+E(c.gl.by||"")+'">↗ In Global notes</span>':'<button class="gh mini" data-a="mkglobal" title="Copy this to Global notes &amp; rules — it applies site-wide">↗ Make global</button>'):"")+"</div>"
...
function headerHtml(){var all=[];GK.forEach(function(g){all=all.concat(rowsOf(g));});var t=tally(all),ct=commentTotals();
  ...
  return '<header class="top"><div class="wrap"><div class="hrow">'
    ...
    +'</div></header><div class="wrap"><div class="navbar"><div class="navin"><nav class="jump">'+nav+'</nav><button class="gh mini" data-a="foldall" title="Open or close every section">'+(anyOpen?"▴ Collapse all":"▾ Expand all")+'</button><span class="save '+cls+'" id="save">'+E(msg)+"</span></div></div>"
    +logHtml()+genHtml()+GROUPS.map(groupHtml).join("")+"</div>";}
...
root.addEventListener("dblclick",function(ev){
  if(ev.target.closest("a"))return;             /* double-clicking a link follows it; edit from the text beside it */
...
root.addEventListener("click",function(ev){
  ...
  if(a==="alllog"){showAllLog=!showAllLog;render();return;}
  if(a==="foldall"){var any=!collapsed.log||!collapsed._||GK.some(function(g){return !collapsed[g];});["log","_"].concat(GK).forEach(function(g){collapsed[g]=any;});render();return;}
  ...
  if(a&&a.charAt(0)==="b"&&sec){var g=sec.dataset.g,ids=Object.keys(sel[g]).filter(function(k){return sel[g][k];});
    ...
    ids.forEach(function(id){var it=item(g,id),s=statusOf(g,id);
      ...
      else if(a==="bg"&&!s.g&&can("g"))setStatus(g,id,"g");
      else if(a==="bf"&&!s.f)setStatus(g,id,"f");
      else if(a==="bclear"){["q","c","g","f"].forEach(function(k){if(stat(it,k))it[k]={at:now(),by:me(),x:1};});logIt("clear-flag",g,id,rowName(g,id),"cleared all");touch(gkey(g));}
      else if(a==="bdel")delRow(g,id);});
    sel[g]={};render();return;}
  var tr=b.closest("tr,.gen");if(!tr)return;var g=tr.dataset.g,id=tr.dataset.i,k=g+":"+id;
  if(a==="q"||a==="c"||a==="g"||a==="f"){setStatus(g,id,a);render();return;}
  ...
  var cm=b.closest(".cmt");if(cm){var cid=cm.dataset.c,rk=k+":"+cid;
    ...
    if(a==="mkglobal"){makeGlobal(g,id,cid);render();return;}
