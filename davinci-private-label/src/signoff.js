...
function histEvents(){var seen={},out=[];
  ...
  GK.concat(["_"]).forEach(function(g){var grp=state[g]||{};for(var id in grp){var it=grp[id],name=g==="_"?"Global notes":rowName(g,id);
    [["q","qbs-approve","qbs-unapprove"],["c","client-approve","client-unapprove"],["f","needs-work","clear-flag"]].forEach(function(k){var st=it[k[0]];if(st&&st.at)add(st.x?k[2]:k[1],g,id,name,st.at,st.by,k[0]==="q"?"qbs":k[0]==="c"?"client":"");});
...
function logHtml(){var all=histEvents(),es=showAllLog?all:all.slice(0,10);
  return '<section class="logsec" id="g-log"><div class="shead"><div class="hrow">'+chev("log")+'<h2>Activity</h2><span class="tct">'+all.length+" actions · everyone, every change</span></div>"
    ...
    +(!collapsed.log?(es.length?'<ul class="log">'+es.map(function(e){return "<li"+(e.d?' class="derived" title="Recovered from the saved item"':"")+"><b>"+E(e.by)+"</b>"+(e.sd?' <i>'+(e.sd==="qbs"?"QBS":"client")+"</i>":"")+" "+E(ACT[e.a]||e.a)+" <em>"+E(e.n)+"</em>"
...
function headerHtml(){var all=[];GK.forEach(function(g){all=all.concat(rowsOf(g));});var t=tally(all),ct=commentTotals();
  ...
  return '<header class="top"><div class="wrap"><div class="hrow">'
    ...
    +'<div class="stats">'+tile("a",t.both,"Fully approved")+tile("q",t.q,"QBS approved")+tile("c",t.c,"Client approved")+tile("f",t.f,"Needs work")+tile("u",t.todo,"Not reviewed")+tile("o",ct.open,"Open comments")+tile("r",ct.done,"Resolved")+tile("t",t.n,"Assets")+"</div>"
